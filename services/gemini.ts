...
export const analyzeCode = async (code: string): Promise<CodeAnalysis> => {
    return retryOperation(async () => {
        ...
        // Using gemini-3-pro-preview with high thinking budget for complex code analysis
        const response = await ai.models.generateContent({
...
export const generateStoryboard = async (prompt: string): Promise<string | null> => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash-image',
      contents: {
        parts: [{ text: `Generate a sequential 4-panel educational comic strip explaining: "${prompt}".
